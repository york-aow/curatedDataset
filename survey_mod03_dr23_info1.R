...
for (col_name in names(survey_mod03_dr23)) {
  ...
  current_class <- class(survey_mod03_dr23[[col_name]])[1] 
  variable_types <- c(variable_types, current_class)
  
  # Get the label attribute
  current_label <- attr(survey_mod03_dr23[[col_name]], "label")
...
survey_mod03_dr23_info <- data.frame(
  Variable_name = variable_names,
  Variable_type = variable_types,
  Variable_label = variable_labels
)
# Save csv
write.csv(survey_mod03_dr23_info, "survey_mod03_dr23_info1.csv", row.names = FALSE)
